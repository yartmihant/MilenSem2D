
Это исследовательские дневники по численному моделированию распространения сейсмических волн в двумерной среде MILEN SEM 2D.

Основу исследования составляют дневники @dev_<P>_<CH>_<T>_*.py где P, CH, T - номера частей, глав и задач дневника. 

---

## Структура проекта

```
MilenSem2D/
├── AGENTS.md                 # Этот файл — правила и структура проекта
├── CHANGELOG.md              # Журнал изменений
├── VERSION                   # Текущая версия проекта
├── README.md                 # Общее описание проекта
├── requirements.txt          # Python-зависимости
│
├── dev_<P>_<CH>_*.py         # Дневники разработки (конвертируются в .ipynb)
├── dev_<P>_<CH>_*_summary.md # Рефераты глав
│
├── src/                      # Исходные данные (ТОЛЬКО ДЛЯ ЧТЕНИЯ)
│   ├── README.md             # Описание всех файлов в src/
│   ├── well1_Backus_Elast_Vp_Vs_rhob.txt     # Каротаж скважины 1
│   ├── well_2_Backus_Elast_param_Vp_Vs_rhob.txt  # Каротаж скважины 2
│   ├── wells_geometry_sketch_corr_.xlsx       # Структура 75 слоёв
│   ├── Координаты скважин.txt                 # Устья скважин
│   ├── Depth_surfaces_for_Fidesys/            # 11 сейсмических горизонтов
│   ├── Sources_coordinates                    # Координаты источников
│   ├── tesseral_example/                      # Референсные SEG-Y Tesseral
│   ├── static_material.csv                    # Статические параметры слоёв
│   └── ...                                    # PDF, Excel, документация
│
├── data/                     # Генерируемые данные (результаты расчётов)
│   ├── dev_1_1_*.npz         # Материалы (интерполяция)
│   ├── dev_1_2_*.npz         # Геометрия сетки
│   ├── dev_1_3_*.fc          # FC модели для Fidesys
│   ├── dev_1_4_*.fds         # FDS модели (адаптивные)
│   ├── dev_1_5_*.npz         # Границы слоёв
│   ├── dev_1_6_*.npz/.fc     # Материал + FC модель (75 слоёв)
│   ├── dev_1_7_*.npz/.fc     # Эйлерова сетка материала
│   ├── dev_2_1_*.sgy         # SEG-Y модели для Tesseral
│   ├── dev_2_3/              # Результаты полноволнового расчёта (227 источников)
│   └── dev_2_4/              # Результаты теста Love Waves
│
├── img/                      # Графические результаты (префикс dev_P_CH_)
│   ├── dev_1_1_*.png         # Интерполяция материалов
│   ├── dev_1_2_*.png         # Геометрия сетки
│   ├── dev_1_3_*.png         # FC модели
│   ├── dev_1_4_*.png         # Адаптивные модели
│   ├── dev_1_5_*.png         # Границы слоёв
│   ├── dev_1_6_*.png         # Материал (новый)
│   ├── dev_1_7_*.png         # Эйлерова сетка
│   ├── dev_2_2_*.png         # Волновое поле (production)
│   └── dev_2_3_animation/    # Анимации волновых полей
│
├── scripts/                  # Расчётные скрипты (Fidesys-SEMGPU)
│   ├── 2_2_seismic_scripts/  # Production-расчёт (227 источников)
│   ├── 2_3_merge_script.py   # Объединение SEG-Y gather
│   └── 2_4_love_wave/        # Тест волн Лява
│
└── fc_model/                 # Вспомогательные модули для FC формата
```

## Содержание дневников

### Часть 1: Построение модели

| Глава | Файл | Тема |
|-------|------|------|
| I.1 | `dev_1_1_material_interpolation.py` | Интерполяция упругих параметров между скважинами |
| I.2 | `dev_1_2_mesh_geometry.py` | Построение геометрии КЭ-сетки по сейсмическим горизонтам |
| I.3 | `dev_1_3_build_fc_model.py` | Сборка FC-модели для Fidesys |
| I.4 | `dev_1_4_adaptive_fds_model.py` | Адаптивная FDS-модель (сплайновая геометрия) |
| I.5 | `dev_1_5_layer_update.py` | Переработка структуры слоёв (75 слоёв из Excel) |
| I.6 | `dev_1_6_material_update.py` | Генерация материала для 75-слойной модели |
| I.7 | `dev_1_7_material_euler.py` | Эйлерова декартова сетка материала (5×5 м) |

### Часть 2: Расчёт и обработка

| Глава | Файл | Тема |
|-------|------|------|
| II.1 | `dev_2_1_segy_material.py` | Конвертация материала в SEG-Y для Tesseral |
| II.2 | `dev_2_2_calculation.py` | Полноволновой расчёт (CPU → GPU SEMGPU) |
| II.3 | `dev_2_3_1_example_headers.py` | Анализ формата SEG-Y заголовков |
|      | `dev_2_3_2_merge_shots.py` | Объединение выстрелов в gather |
| II.4 | `dev_2_4_love_waves.py` | Калибровочный тест: волны Лява |

---

Правила ведения дневника:

1. Придерживайся в оформлении дневника строго императивного стиля Jupyter-style. Не создавай главных функций main() или подобных ей. Оформляй документ согласно правилам .cursor/rules/PY_FORMAT_RULES.mdc. Помни - этот код будет конвертироваться в ipynb-блокноты.
2. Всегда помещай генерируемые скриптами файлы данных в папку data и называй по имени текущей. Например, @data/dev_1_1_model1_material.npz. Файлы картинок помещай в папку img и называй так же начиная с имени текущей части/главы/задачи. Временные файлы помещай в корень проекта, что бы можно было легко заметить их и удалить.
3. Начинай каждую главу с описания текущей задачи, например:

```python
""" # Численное моделирование распространения сейсмических волн в двумерной среде MILEN SEM 2D. Часть первая."""

""" ## Глава I: Интерполяция упругих параметров материалов """

"""
### Задача

Интерполяция упругих параметров (Vp, Vs, плотность) между двумя скважинами для создания 2D модели. Сравнение линейной интерполяции и послойного кригинга.
"""
```

4. Каждая глава должна заканчиваться комментарием с выводом: что сделано.

5. Каждая глава должна иметь summary: @dev_<P>_<CH>_*_summary.md , где дается её краткий реферат с описанием сделанной работы, включая все файлы на входе и на выходе.

6. Запрещено пытаться менять файлы в папке src - эти данные только для чтения.

7. Игнорируй файлы *.ipynb - они создаются пользователем из соответствующих файлов *.py и читать или писать их не нужно


---


# Правила оформления Python файлов для конвертации в Jupyter

Этот документ содержит правила форматирования Python файлов, которые гарантируют корректную конвертацию в Jupyter ноутбуки с помощью инструмента `py2jupyter`.

## Основные принципы

### 1. Структурирование файла

- **Начинайте с заголовка файла**: Используйте специальный формат заголовков для обозначения начала разделов
- **Группируйте связанный код**: Логически связанные блоки кода должны быть вместе
- **Используйте пустые строки**: Разделяйте логические блоки пустыми строками (но не в начале/конце функций)

### 2. Заголовки и разделы

#### Формат заголовков разделов
```python
""" # Заголовок раздела # """
```

**Правила:**
- Заключен в тройные кавычки с пробелами: `""" # текст # """`
- Можно использовать префикс `r` если нужен: `r""" # текст # """`
- Становится markdown ячейкой с заголовком `# Заголовок раздела #`
- Используется для четкого структурирования больших скриптов
- **Важно**: обычные Python комментарии `# текст` остаются комментариями!

#### Примеры:
```python
""" # Численные методы решения ОДУ на примере затухающего гармонического осциллятора # """

"""
На этом занятии мы реализуем и сравним ряд численных методов интегрирования:
- Метод Leapfrog-Verlet
- Метод Рунге-Кутты
- Метод Адамса-Бэшфорта
- Метод Адамса-Моултона
"""

""" ## Уравнение гармонического осциллятора ## """
```

### 3. Многострочные комментарии и описания

#### Markdown комментарии
```python
r"""
Формула: $E = \frac{1}{2}mv^2 + \frac{1}{2}kx^2$
Интеграл: $\int_0^1 x dx = \frac{1}{2}$
"""

"""
Обычное описание без специальных символов.
Может содержать списки, жирный текст и другие элементы markdown.
"""
```

**Правила:**
- Поддерживаются как `r"""..."""`, так и `"""..."""`
- При обратной конвертации префикс `r` **автоматически добавляется** при наличии `\` символов
- Тройные кавычки на отдельных строках
- Становится markdown ячейкой
- Поддерживает полный синтаксис Markdown и LaTeX

### 4. Структура кода

#### Импорты
```python
import numpy as np
import matplotlib.pyplot as plt
from math import pi
import matplotlib.animation as animation
from scipy.optimize import fsolve

from oscillator_plots import plots
```

**Правила:**
- Группируйте импорты в начале файла
- Разделяйте стандартные библиотеки, сторонние пакеты и локальные модули
- Импорты автоматически попадут в отдельную code ячейку

#### Глобальные переменные и константы
```python
""" Физические параметры гармонического осциллятора """

m = 1.0   # масса, кг
b = 0.01  # коэффициент трения, кг/с
k = 4.0   # жесткость пружины, Н/м

""" Начальные условия """

u0 = 0.5   # начальное смещение, м
v0 = 0.0   # начальная скорость, м/с
```

**Правила:**
- Предваряйте группы констант заголовком раздела
- Добавляйте комментарии к каждой переменной
- Группируйте логически связанные константы

#### Функции и классы

```python
def energy(u, v):
    """
    Расчет полной энергии гармонического осциллятора

    Args:
        x: смещение, м
        v: скорость, м/с

    Returns:
        E: полная энергия, Дж
    """
    # Потенциальная энергия
    U = 0.5 * k * u**2
    # Кинетическая энергия
    K = 0.5 * m * v**2
    return U + K
```

**Правила:**
- **Каждая функция верхнего уровня → отдельная code ячейка**
- **Docstring'и внутри функций/классов остаются частью кода** (важно!)
- Вложенные функции остаются в одной ячейке с родительской
- Функции должны иметь понятные имена и подробные docstring'и

### 5. Комментарии в коде

#### Magic команды Jupyter
```python
#%matplotlib inline

#%config InlineBackend.figure_format = 'retina'

#%time
```

**Правила:**
- **Magic команды** начинаются с `#%` в Python файле
- Становятся **отдельными code ячейками** в Jupyter ноутбуке
- При обратной конвертации **одиночные magic ячейки** становятся комментариями `#%command`
- Поддерживают все стандартные magic команды Jupyter (`%matplotlib`, `%time`, `%config` и т.д.)

#### Shell команды Jupyter
```python
#! pip install seaborn

#! pip install --upgrade numpy matplotlib

#! ls -la
```

**Правила:**
- **Shell команды** начинаются с `#!` в Python файле
- Становятся **отдельными code ячейками** в Jupyter ноутбуке
- При обратной конвертации **одиночные shell ячейки** становятся комментариями `#!command`
- Поддерживают все shell команды (`pip install`, `ls`, `cd`, `mkdir` и т.д.)

#### Обычные комментарии
```python
# Параметры затухающего осциллятора
omega = np.sqrt(k / m)  # собственная частота
gamma = b / (2 * m)      # коэффициент затухания

# Определяем тип колебаний
discriminant = gamma**2 - omega**2
```

**Правила:**
- **Обычные комментарии `# текст` остаются Python комментариями**
- Используйте их для пояснения сложной логики
- Не переусердствуйте - код должен быть самодокументируемым

#### Разница между типами комментариев
```python
# Это КОММЕНТАРИЙ - станет частью code ячейки
"""
Это markdown блок - станет отдельной markdown ячейкой
"""

def my_function():
    """Это DOCSTRING - останется в коде функции"""
    return "result"
```

### 6. Логическое разделение кода

#### Императивный Jupyter-like стиль
```python
""" ## 1. Решаем задачу аналитическим методом ## """

def analytical_method(u0, v0, dt, n_steps):
    """Реализация аналитического метода"""
    # код функции
    return u_result, v_result

# Выполнение и вывод результатов
u_analytic, v_analytic = analytical_method(u0, v0, dt, n_steps)
energy_analytic = energy(u_analytic, v_analytic)
print(f"Аналитическое решение: энергия = {energy_analytic:.4f}")


""" ## 2. Решаем задачу методом Leapfrog ## """

def leapfrog_method(u0, v0, dt, n_steps):
    """Реализация метода Leapfrog"""
    # код функции
    return u_result, v_result

# Выполнение и вывод результатов
u_leapfrog, v_leapfrog = leapfrog_method(u0, v0, dt, n_steps)
energy_leapfrog = energy(u_leapfrog, v_leapfrog)
print(f"Метод Leapfrog: энергия = {energy_leapfrog:.4f}")


""" ## 3. Решаем задачу методом Рунге-Кутты ## """

def rk4_method(u0, v0, dt, n_steps):
    """Реализация метода Рунге-Кутты"""
    # код функции
    return u_result, v_result

# Выполнение и вывод результатов
u_rk4, v_rk4 = rk4_method(u0, v0, dt, n_steps)
energy_rk4 = energy(u_rk4, v_rk4)
print(f"Метод Рунге-Кутты: энергия = {energy_rk4:.4f}")
```

**Правила:**
- **Строго императивный стиль**: комментарий в тройных кавычках → определение функции → немедленное выполнение с выводом результатов
- **Не используйте функцию main()** - весь исполняемый код должен быть на верхнем уровне
- **Каждая логическая задача** оформляется как: объяснение → функция → выполнение → вывод
- **Функции определяются непосредственно перед их использованием**
- **Две пустые строки подряд** на уровне нулевого отступа разделяют код на отдельные ячейки


### 7. Финальные замечания и выводы

```python
""" ## Выводы ## """

"""
На основе проведенного сравнения методов интегрирования...

### По точности решения:
1. **Методы Рунге-Кутты (RK4)** - наиболее точный одношаговый метод
2. **Методы Адамса-Моултона (неявные)** - самые точные для жестких систем

### Рекомендации по применению:
- **Для высокой точности на коротких интервалах**: RK4
- **Для длинных расчетов с умеренной точностью**: Leapfrog
"""
```

**Правила:**
- Завершайте файл разделом с выводами или заключением
- Используйте markdown для структурирования текста
- Добавляйте списки, таблицы и формулы LaTeX

## Шаблон правильно оформленного файла

```python
""" # Название проекта/задачи # """

"""
Краткое описание проекта, цели, используемые методы.
"""

# Импорты
import numpy as np
import matplotlib.pyplot as plt

# Глобальные константы
""" ## Параметры системы ## """

param1 = 1.0
param2 = 2.0


""" ## 1. Первая задача - подготовка данных ## """

def prepare_data(input_file):
    """Подготовка данных из файла"""
    # код функции
    return processed_data

# Выполнение и проверка
data = prepare_data('input.txt')
print(f"Загружено {len(data)} записей")


""" ## 2. Вторая задача - обработка данных ## """

def process_data(data, params):
    """Обработка данных с заданными параметрами"""
    # код функции
    return result

# Выполнение и вывод результатов
result = process_data(data, param1)
print(f"Результат обработки: {result}")


""" ## 3. Третья задача - визуализация ## """

def create_plot(result, output_file):
    """Создание графика результатов"""
    # код функции
    plt.savefig(output_file)

# Выполнение и сохранение
create_plot(result, 'output.png')
print("График сохранен в output.png")


""" ## Выводы ## """

"""
Анализ полученных результатов...
Рекомендации...
"""
```

## Важные напоминания

1. **Не используйте пустые строки в начале/конце функций** - они будут автоматически удалены при конвертации
2. **Docstring'и должны быть внутри функций**, а не перед ними
3. **Markdown блоки должны быть отдельными** от кода функций
4. **Тестируйте конвертацию** - всегда проверяйте цикл py → ipynb → py
5. **Используйте описательные имена** для функций и переменных
6. **Добавляйте LaTeX формулы** в `r"""` блоках для математических выражений

---


## Список изменений проекта (`CHANGELOG.md`)

### Правила работы ИИ по идеологии GitPlus

Если вы — ИИ-ассистент, работающий над этим проектом, вы обязаны соблюдать следующие правила управления изменениями и версиями.

#### 1. Фиксация изменений (Changelog)

Вы должны фиксировать проделанную работу в файле `CHANGELOG.md`

- **Проверка наличия блока [Unreleased]:**
  Проверьте, существует ли в `CHANGELOG.md` заголовок версии с меткой `[Unreleased]`.
  
- **Если блок [Unreleased] существует:**
  Добавьте описание ваших изменений в соответствующие секции (Added, Changed, Fixed и т.д.) этого блока.

- **Если блока [Unreleased] нет:**
  Создайте новый блок версии над последним существующим блоком.

#### 2. Создание нового блока версии

При создании нового блока:
1. Считайте текущую версию из файла `VERSION` (например, `1.2.3`).
2. Инкрементируйте патч-версию (последнее число) на +1 (станет `1.2.4`), не меняя `VERSION`.
3. Добавьте в `CHANGELOG.md` новый заголовок вида:
   ```markdown
   ## 1.2.4 [YYYY-MM-DD HH:MM:SS] [Unreleased]
   ```
   (подставьте актуальную дату и время).
4. Ниже добавьте категории изменений (Добавлено, Изменено, Исправлено, Убрано) и описание проделанной вами и пользователем незафиксированной работы.

#### Запрещено использовать git-команды изменения

**Важно**: ИИ-агентам запрещено выполнять git-команды, изменяющие состояние репозитория, рабочее дерево, индекс, ветки, теги, историю коммитов или удалённые репозитории.  Это инструмент контроля пользователя над проектом, и право его использования остается исключительно за пользователем. 
  
Запрещены, в частности:  
`git add`, `git commit`, `git restore`, `git checkout`, `git switch`, `git reset`, `git revert`, `git merge`, `git rebase`, `git cherry-pick`, `git tag`, `git stash`, `git push`, `git pull`, `git fetch`.  Применение таких команд допустимо только при явном прямом недвусмысленном приказе пользователя.
  
Разрешены read-only команды для анализа текущего состояния проекта, например:  
`git status`, `git diff`, `git diff --staged`, `git log`, `git show`, `git blame`, `git rev-parse HEAD`, `git branch --show-current`.  `git diff` и `git status` предпочтительны, когда агенту нужно понять контекст уже внесённых локальных изменений.

Даже в разрешённом режиме git используется только как инструмент чтения и анализа.  
Любые действия по фиксации, откату, синхронизации или переписыванию истории остаются исключительно за пользователем.

