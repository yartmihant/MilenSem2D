...
def create_depth_maps(surfaces_data):
    ...
    output_filename = 'img/dev_1_2_depth_maps_all_surfaces.png'
...
def save_profile_sections_data(all_profiles, profile_depths, profile_grid, surface_names, wells_final, profile):
    ...
    # Сохраняем файл
    output_filename = 'data/dev_1_2_profile_sections_depths.npz'
...
def plot_all_profiles_combined(profile_depths, profile_grid, wells_final, profile):
    ...
    # Сохраняем график
    output_filename = 'img/dev_1_2_profile_sections_combined.png'
    plt.savefig(output_filename, dpi=300, bbox_inches='tight')
    if SHOW_PLOTS:
        plt.show()
    else:
...
def visualize_grid(x_coords, grid_z_coords, grid_params, model_layers):
    ...
    # Сохраняем график
    output_filename = 'img/dev_1_2_grid_structure_visualization.png'
    plt.savefig(output_filename, dpi=300, bbox_inches='tight')
    if SHOW_PLOTS:
        plt.show()
    else:
...
def save_grid_results(x_coords, grid_z_coords, grid_params, model_layers):
    ...
    grid_file = 'data/dev_1_2_model1_mesh_coords.npz'
...
В данной главе была построена деформированная конечно-элементная сетка с адаптацией к геологическим границам на основе 11 сейсмических поверхностей. 

Были выполнены следующие задачи:
1. Загрузка и анализ геологических поверхностей из файлов `src/Depth_surfaces_for_Fidesys/hrz__*.txt`
2. Создание профильных сечений и построение карт глубин
3. Адаптация регулярной сетки к геологическим границам
4. Построение многослойной структуры с 18 геологическими слоями

Результаты сохранены в файлах:
- `data/dev_1_2_profile_sections_depths.npz` - данные профильных сечений
- `data/dev_1_2_model1_mesh_coords.npz` - координаты адаптивной сетки
- Визуализации в папке `img/dev_1_2_*.png`
"""