...
Создаем декартову сетку дискретизации 5x5 м. Это соответствует размеру ячейки для метода конечных разностей, достаточному, чтобы улавливать медленные (~500 м/с) волны с частотой 30 Hz. Эту же сетку без пространственного прореживания записываем в FC: каждому материалу передаем все относящиеся к его слою точки 5x5 м.
"""
