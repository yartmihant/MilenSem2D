...
create porepressure on surface 1  value 0
create formula 1 'exp(-0.5*sqrt((x-4880)^2+(y-600)^2)/4)/(2.5066*4)*ricker(1e+10, 30, 0, time)'
...
create material 1
modify material 1 name 'TestMat'
modify material 1 set property 'DENSITY' value 1823.78
modify material 1 set property 'BIOT_ALPHA' value 1
modify material 1 set property 'MODULUS' value 8.86244e+09
modify material 1 set property 'POISSON' value 0.283926
block 1 add surface all
block 1 material 1 cs 1 category plane order 2

analysis type dynamic elasticity dim2 planestrain preload off
dynamic method full_solution scheme explicit maxsteps 10000 maxtime 0.25
dynamic results everytime 0.05
output nodalforce off energy off record3d off material on without_smoothing off fullperiodic off

calculation start path '/home/antonov/Документы/CAE-Fidesys-8.1/dev_2_1_milen2Do2_pore_center_simple.pvd'

"""

"""
Поскольку материал однородный, а сетка - не очень, значения скорости волны подобраны так, что бы она была в 2 раза быстрее минимальной (поверхностной) (удолетворяла характерному размеру элемента сетки в глубинной части модели). В обмен на это мы будем добиваться отсутствия помех 1% по нижней части модели. 

Примечание: таблицы Уханова составлялись для идеально ровной сетки, ровного материала и другого типа сигнала. Поэтому мы опираемся на них как на нижнюю оценку, но делаем перекалибровку.
...
create porepressure on surface 1  value 0
create formula 1 'exp(-0.5*sqrt((x-5875)^2+(y-5)^2)/2)/(2.5066*2)*ricker(1e+10, 30, 0, time)'
modify porepressure 1  formula 1

analysis type dynamic elasticity dim2 planestrain preload off
dynamic method full_solution scheme explicit maxsteps 10000 maxtime 0.5
dynamic results everytime 0.05
output nodalforce off energy off record3d off material on without_smoothing off fullperiodic off
